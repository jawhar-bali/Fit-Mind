...
def speak_count(count):
    # Speak out the push-up count
    engine = pyttsx3.init()
    engine.say("{}".format(count))
    if count == 10:
        engine.say("wow good")
    elif count == 20:
        engine.say("perfect")
...
# For webcam input:
  ...
  with mp_pose.Pose(
   ...
   while cap.isOpened():
      ...
      cv2.circle(image,rightWrist,15,(0,255,255),15)
      cv2.circle(image,rightShoulder,15,(0,255,255),15)
      ...
      if distanceCalculate(rightShoulder, rightWrist) > 150:
        pushUpStart = 1
        print('DDDDD')  # Debugging message
        print(pushUpStart)  # Debugging message
      elif pushUpStart and distanceCalculate(rightShoulder, rightWrist) < 70:
        pushUpCount = pushUpCount + 1
        pushUpStart = 0
        print('HHHHHH')  
        output_thread = threading.Thread(target=speak_count, args=(pushUpCount, ))
        output_thread.start()
        print('CCCCCC') 
 ...
 # Line thickness of 2 px
      ...
      image = cv2.putText(image, "Push-up:" + str(pushUpCount), org, font, fontScale, color, thickness, cv2.LINE_AA)
